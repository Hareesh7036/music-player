...
export const likesRoutes = new Elysia({ prefix: "/api/likes" })
  ...
  .get("/user/:userId", async ({ params: { userId } }) => {
    try {
      ...
      console.log(
        `📋 Fetching ${user.favoritesSongs.length} liked songs for user ${userId}`
      );
      return {
        success: true,
      ...
      return {
        success: false,
        error: `Failed to fetch liked songs: ${errorMessage}`,
      };
    }
  })
  ...
  .get("/ids/:userId", async ({ params: { userId } }) => {
    try {
      ...
      return {
        success: false,
        error: `Failed to fetch liked song IDs: ${errorMessage}`,
      };
    }
  });
